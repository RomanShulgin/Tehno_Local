...
class OrderLight extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return new RaisedButton(
      ...
      onPressed: () {
        Navigator.pushNamed(context, '/Order', arguments: <String, String>{
          'date': struct['Дата'],
          'number': struct['Номер'],
          'type': 'Order'
        });
      },
      child: Text('№' + struct['Номер'] + ' от ' + struct['Дата'],
          textScaleFactor: 0.8),
    );
  }
}
...
class OrderState extends State<Order> {
  ...
  void Pay(struct) {
    if (getParam(json, "Оплачен") == 'Оплачен') {
      ...
      buttons.add(FlatButton(
        ...
        onPressed: () {
          Navigator.push(context,
              MaterialPageRoute(builder: (context) => Payment(struct)));
        },
      ));
      buttons.add(FlatButton(
          child: Text('Возврат'),
      ...
      Navigator.push(
          context, MaterialPageRoute(builder: (context) => Payment(struct)));
    }
  }
      ...
      bordersList.add(TableRow(children: [
        ...
        TableCell(
          ...
          children: <Widget>[
            ...
            Text(doc['Цена'], textScaleFactor: 0.8)
          ],
        )),
          ...
          body: ListView(
            children: [
              ...
              Row(children: [
                LGroup(
                    Check(
                        getParam(json, "Оплачен") == 'Оплачен' ? 'Да' : 'Нет'),
                    'Оплата'),
                LGroup(
                    Check(getParam(json, "Отгружен") == 'Отгружен'
                        ? 'Да'
                        : 'Нет'),
                    'Отгрузка'),
  ...
  List getOrders(strings) {
    ...
    for (var doc in strings) {
      if (doc['ЕстьЗаказ']) {
        bordersList.add(TableRow(children: [
          TableCell(
              child: KPMicro({'Номер': doc['НомерКП'], 'Дата': doc['ДатаКП']}),
              verticalAlignment: TableCellVerticalAlignment.middle),
          TableCell(
            child: OrderMicro({'Номер': doc['Номер'], 'Дата': doc['Дата']}),
            verticalAlignment: TableCellVerticalAlignment.middle,
          ),
          TableCell(
            child: Column(
              children: [
                Check(doc["Оплачен"] == 'Оплачен' ? 'Да' : 'Нет', 12.0),
                Check(doc["Отгружен"] == 'Отгружен' ? 'Да' : 'Нет', 12.0),
                Check(doc["Документы"], 12.0)
              ],
            ),
          ),
          ...
          TableCell(
              child: Text(doc['СуммаДокумента'], textScaleFactor: 0.9),
              verticalAlignment: TableCellVerticalAlignment.middle),
          TableCell(
              child: Text(
                doc['Комментарий'],
