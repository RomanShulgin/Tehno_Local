...
class GoodsMicro extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return new InkWell(
      onTap: (){dialogGoodsCard(struct['Код'],'Coded',context);},
      child: Center(child:
      Container(
      ...
      child: Column(children: <Widget>[Text(struct['Номенклатура'],textScaleFactor: 0.8,), ],) ,
    )
    ),
    );
...
class NomiconState extends State<Nomicon> {
  ...
  String getGoodsPicture(code)
  {
    ...
    return 'http://46.34.155.26/dist/hs/GetBasicInfo/$user/$password/goodsicon/?code=$code';
  }
  ...
  List paramList(){
    ...
    for(var par in params.entries){
      listparams.add(
          TableRow(
              children:[
                TableCell(child:ContSmall(Text(par.value['ИмяПараметра'],textScaleFactor: 0.8))), //.['ИмяПараметра']
                TableCell(child: ContSmall(Text(par.value['Значение'],textScaleFactor: 0.8))),
              ]
          )
      );
    }
    ...
    if (isloading) {
    ...
    else
      {
        ...
        return Container(
            ...
            LGroup(Text(name),"Наименование"),//getParam(json,"Наименование")),"Наименование"),
            Group(Text(getParam(json,"Цена")),"Цена"),
            Group(Text(getParam(json,"Наличие")),"Наличие"),
            LGroup(BasketAdd(getParam(json,"Код"),name,getParam(json,"Цена"),fixparams),"Корзина"),
            Group(
                Table(
              ...
              children: paramList()//[TableRow(children: [TableCell(child: Text("1"))])]//paramList(),
            ), "Параметры"),
            /*RButton('Назад', () {
              Navigator.of(context).pop();
            })*/
